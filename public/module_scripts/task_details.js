$(document).ready(function () {
	...
	$("#update_task_progress").submit(function (e) {
		/*Form Submit*/
		e.preventDefault();
		var obj = $(this), action = obj.attr('name');
		$.ajax({
	...
	$(".delete_note").on("click", function () {
		...
		$('.note_option_id_' + field_id).fadeOut();
		$.ajax({
			url: main_url + "delete-task-note",
			type: "GET",
			data: 'jd=1&data=task_note&field_id=' + field_id,
	...
	$(".delete_file").on("click", function () {
		var field_id = $(this).data('field');
		$.ajax({
			url: main_url + "delete-task-file",
			type: "GET",
			data: 'jd=1&data=task_file&field_id=' + field_id,
			success: function (response) {
				if (response) {
