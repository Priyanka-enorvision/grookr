$(document).ready(function () {
	...
	$('.edit-modal-data').on('show.bs.modal', function (event) {
		...
		var modal = $(this);
		$.ajax({
			url: main_url + "read-warning",
			type: "GET",
			data: 'jd=1&data=warning&field_id=' + field_id,
	...
	$('.view-modal-data').on('show.bs.modal', function (event) {
		...
		$.ajax({
			url: main_url + "read-warning",
			type: "GET",
			data: 'jd=1&type=view_warning&field_id=' + field_id,
			success: function (response) {
				if (response) {
...
$(document).on("click", ".delete", function () {
	...
	$('#delete_record').attr('action', main_url + 'delete-warning');
});