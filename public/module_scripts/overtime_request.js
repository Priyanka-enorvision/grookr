$(document).ready(function () {
	var xin_table = $('#xin_table').DataTable({
		"bDestroy": true,
		"ajax": {
			url: main_url + "overtime-request-datalist",
			type: 'GET'
		},
		"language": {
		...
		$.ajax({
			...
			success: function (JSON) {
				if (JSON.error != '') {
					...
					window.location.href = main_url + 'overtime-request';
				}
			},
	...
	$('.view-modal-data').on('show.bs.modal', function (event) {
		...
		var modal = $(this);
		$.ajax({
			url: main_url + 'read-overtime-request',
			type: "GET",
...
$(document).on("click", ".delete", function () {
	...
	$('#delete_record').attr('action', main_url + 'delete-overtime');
});
