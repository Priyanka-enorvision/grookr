...
<div class="form-group" id="staff_ajax">
  ...
  <select class="form-control" name="employee_id" data-plugin="select_hrm" data-placeholder="<?= lang('Dashboard.dashboard_employee'); ?>">
    ...
    <?php foreach ($staff_details as $staff) { ?>
      <?php
      $staff_info = $UsersModel->where('user_id', $staff['user_id'] ?? '')
        ->where('user_type', 'staff')
        ->first();
      // Only show option if staff_info exists and has required fields
      if ($staff_info && isset($staff_info['user_id'], $staff_info['first_name'], $staff_info['last_name'])):
      ?>
        <option value="<?= htmlspecialchars($staff_info['user_id']) ?>">
