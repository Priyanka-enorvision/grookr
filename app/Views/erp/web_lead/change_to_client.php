...
?>
    <div class="modal-header">
        <h5 class="modal-title"><?= lang('Main.xin_change_to_client');?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
    </div>
    
    <?php
    // Set attributes and hidden fields for the form
    $attributes = ['name' => 'convert_client', 'id' => 'convert_client', 'autocomplete' => 'off', 'class' => 'm-b-1'];
    $hidden = ['_method' => 'EDIT', 'token' => $field_id];
    echo form_open('erp/dashboard/convert_client', $attributes, $hidden);
    ?>
    ...
    <script type="text/javascript">
        $(document).ready(function() {		
            ...
            $("#convert_client").submit(function(e) {
                e.preventDefault();
                var obj = $(this);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('input[name="csrf_token"]').val(response.csrf_hash);
                            ...
                            $('#convert_client').closest('.modal').modal('hide');
                            setTimeout(function() {
                    ...
                    complete: function() {
                        ...
                        setTimeout(function() {
                            console.log('Redirecting to:', '<?= site_url('erp/web-leads-list'); ?>'); 
                            window.location.href = '<?= site_url('erp/web-leads-list'); ?>';
                        }, 2000);
                    }
                });
