...
?>
  ...
  <?php echo form_open('erp/add-attendance', $attributes, $hidden); ?>
          ...
          <div class="col-md-12">
            <div class="form-group">
              ...
              <div class="input-group">
                <input class="form-control " placeholder="<?= lang('Attendance.xin_attendance_date'); ?>" name="attendance_date_m" type="date" value="<?php echo date('Y-m-d'); ?>">
              </div>
            </div>
          </div>
        </div>
...
<?php
  ...
  $segment_id = $request->getGet('field_id');
  $ifield_id = udecode($segment_id);
  $result = $TimesheetModel->where('time_attendance_id', $ifield_id)->first();
  $manualRecordData = $UsersModel->where('user_id', $result['employee_id'])->first();
...
?>
  ...
  <?php echo form_open('erp/update-attendance-record', $attributes, $hidden); ?>
  <div class="modal-body">
    <div class="row">
      <div class="col-md-12">
        <?php if ($user_info['user_type'] == 'company') { ?>
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                ...
                <select class="form-control" name="employee_id" data-plugin="select_hrm" data-placeholder="<?= lang('Dashboard.dashboard_employee'); ?>">
                  <?php foreach ($staff_info as $staff) { ?>
                    <option value="<?= $staff['user_id'] ?>" <?= ($staff['user_id'] === $manualRecordData['user_id']) ? 'selected' : ''; ?>>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="form-group">
              ...
              <div class="input-group">
                <input class="form-control " placeholder="<?= lang('Attendance.xin_attendance_date'); ?>" name="attendance_date_m" type="date" value="<?php echo $result['attendance_date']; ?>">
              </div>
            </div>
          </div>
        </div>
