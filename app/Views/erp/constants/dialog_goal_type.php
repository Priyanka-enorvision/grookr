...
$get_animate = '';
if ($request->getGet('data') === 'goal_type' && $request->getGet('field_id')) {
	$category_id = udecode($field_id);
	$result = $ConstantsModel->where('constants_id', $category_id)->where('type', 'goal_type')->first();
?>

	<div class="modal-header">
		<h5 class="modal-title">
			<?= lang('Performance.xin_hr_edit_goal_type'); ?>
		...
		<div class="row">
			<div class="col-md-12">
				<div class="form-group">
					...
					<input type="text" class="form-control" name="name" placeholder="<?= lang('Dashboard.xin_hr_goal_tracking_type'); ?>" value="<?= $result['category_name']; ?>">
				</div>
			</div>
		</div>
	</div>
				...
				$.ajax({
					...
					success: function(JSON) {
						if (JSON.error != '') {
							...
							window.location.href = main_url + 'goal-type';
						}
					},
