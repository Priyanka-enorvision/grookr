...
$get_animate = '';
if ($request->getGet('data') === 'expense_type' && $request->getGet('field_id')) {
	$category_id = udecode($field_id);
	$result = $ConstantsModel->where('constants_id', $category_id)->where('type', 'expense_type')->first();
?>

	<div class="modal-header">
		<h5 class="modal-title">
			<?= lang('Finance.xin_edit_category'); ?>
				...
				$.ajax({
					...
					success: function(JSON) {
						if (JSON.error !== '') {
							...
							window.location.href = main_url + 'expense-type';
