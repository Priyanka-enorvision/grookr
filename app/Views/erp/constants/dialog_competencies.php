...
$get_animate = '';
if ($request->getGet('data') === 'competencies' && $request->getGet('data_option') === 'competencies' && $request->getGet('field_id')) {
	$category_id = udecode($field_id);
	$result = $ConstantsModel->where('constants_id', $category_id)->where('type', 'competencies')->first();
?>

	<div class="modal-header">
		<h5 class="modal-title">
			<?= lang('Performance.xin_talent_edit_technical_category'); ?>
...
<?php } else if ($request->getGet('data') === 'competencies' && $request->getGet('data_option') === 'competencies2' && $request->getGet('field_id')) {
	$category_id = udecode($field_id);
	$result = $ConstantsModel->where('constants_id', $category_id)->where('type', 'competencies2')->first();
?>

	<div class="modal-header">
		<h5 class="modal-title">
			<?= lang('Performance.xin_talent_edit_org_category'); ?>
			<span class="font-weight-light">
				<?= lang('Main.xin_information'); ?>
			</span> <br>
			<small class="text-muted">
					...
					success: function(JSON) {
						if (JSON.error != '') {
							...
							var xin_org_table = $('#xin_org_table').dataTable({
								"bDestroy": true,
								"ajax": {
									url: "<?= site_url("erp/types/org_competencies_list") ?>",
									type: 'GET'
								},
