...
$get_animate = '';
if ($request->getGet('data') === 'brand' && $request->getGet('field_id')) {
	$category_id = $field_id;
	$result = $ConstantsModel->where('constants_id', $category_id)->where('type', 'assets_brand')->first();
?>

	<div class="modal-header">
		<h5 class="modal-title">
			<?= lang('Asset.xin_edit_assets_brand'); ?>
		...
		<div class="row">
			<div class="col-md-12">
				<div class="form-group">
					<label for="name">
						<?= lang('Asset.xin_asset_brand'); ?> <span class="text-danger">*</span>
					</label>
					<input type="text" class="form-control" name="name" placeholder="<?= lang('Asset.xin_asset_brand'); ?>" value="<?= $result['category_name']; ?>">
				</div>
			</div>
		</div>
	</div>
				...
				$.ajax({
					...
					success: function(JSON) {
						if (JSON.error !== '') {
							...
							window.location.href = main_url + 'assets-brand';
