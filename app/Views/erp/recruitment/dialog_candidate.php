...
if ($request->getGet('data') === 'candidate' && $request->getGet('field_id')) {
	...
	if ($user_info['user_type'] == 'staff') {
		...
		$staff_info = $UsersModel->where('company_id', $usession['sup_user_id'])->where('user_id!=', $usession['sup_user_id'])->where('user_id!=', $result['staff_id'])->where(['user_type' => 'staff', 'is_active' => 1])->findAll();
	}
?>
	...
	<?= form_open('erp/update-candidate-status', $attributes, $hidden); ?>
		...
		<div class="row">
			...
			<div class="col-md-6 reject_opt">
				<div class="form-group">
					...
					<input class="form-control " placeholder="<?php echo lang('Recruitment.xin_interview_date'); ?>" name="interview_date" type="date" value="">
				</div>
			</div>
			<div class="col-md-6 reject_opt">
				<div class="form-group">
				...
				$.ajax({
					...
					success: function(JSON) {
						if (JSON.error != '') {
							...
							window.location.href = main_url + 'candidates-list';
						}
					},
...
?>
	...
	<?php $hidden = array('_method' => 'EDIT', 'token' => $field_id); ?>
	<?= form_open('erp/update-interview-status', $attributes, $hidden); ?>
	<div class="modal-body">
		<div class="row">
			<div class="col-md-12">
				<div class="form-group">
				...
				$.ajax({
					...
					success: function(JSON) {
						if (JSON.error != '') {
							...
							toastr.success(JSON.result);
							$('input[name="csrf_token"]').val(JSON.csrf_hash);
							window.location.href = main_url + 'jobs-interviews';
						}
					},
					error: function(xhr, status, error) {
						toastr.error('Something went wrong. Please try again.');
