...
<div id="accordion">
  <div id="add_form" class="collapse add-form " data-parent="#accordion" style="">
    ...
    <?= form_open_multipart('erp/add-employee', $attributes, $hidden);?>
    <div class="row">
      <div class="col-md-8">
        <div class="card">
        ...
        <div class="card">
          ...
          <div class="card-body py-2">
            <div class="row">
              <div class="col-md-12">
                <label for="logo">
                  <?= lang('Main.xin_e_details_profile_picture');?></label>
...
<div class="row justify-content-center"> 
  ...
  <?php
    ...
    $status = $r['is_active'] == 1 
        ? '<span class="badge badge-success">'.lang('Main.xin_employees_active').'</span>'
        : '<span class="badge badge-danger">'.lang('Main.xin_employees_inactive').'</span>';

    // Get role information safely
    $role = $RolesModel->where('role_id', $r['user_role_id'] ?? 0)->first() ?? ['role_name' => 'N/A'];
    
    // Get employee details safely
    $employee_detail = $StaffdetailsModel->where('user_id', $r['user_id'] ?? 0)->first() ?? [];
    
    // Get office shift information safely
    $office_shifts = ['shift_name' => 'N/A'];
    if (!empty($employee_detail['office_shift_id'])) {
        $shift = $ShiftModel->where('office_shift_id', $employee_detail['office_shift_id'])->first();
        if ($shift) {
            $office_shifts = $shift;
        }
    }
    
    // Get designation information safely
    $designation = ['designation_name' => 'N/A'];
    if (!empty($employee_detail['designation_id'])) {
        $desig = $DesignationModel->where('designation_id', $employee_detail['designation_id'])->first();
        if ($desig) {
            $designation = $desig;
        }
    }
    ...
    $profile_photo = 'default/user.png'; // default image
    if (!empty($r['profile_photo']) && file_exists(FCPATH.'uploads/users/thumb/'.$r['profile_photo'])) {
        $profile_photo = 'uploads/users/thumb/'.$r['profile_photo'];
    }
  ?>
    ...
    <div class="card user-card user-card-1 mt-4">
      <div class="card-body pt-0">
          ...
          <div class="row align-items-end">
            ...
            <div class="col">
              <img class="img-radius img-fluid wid-80" 
                   src="<?= base_url('public/'.$profile_photo) ?>" 
                   alt="<?= htmlspecialchars($r['first_name'].' '.$r['last_name']) ?>">
            </div>
            <div class="col text-right pb-3">
              ...
              <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                  ...
                  <a class="dropdown-item" href="<?= site_url('erp/employee-details/'.uencode($r['user_id'])) ?>">
                    <i class="feather icon-eye"></i>
                    <?= lang('Main.xin_view') ?>
                  </a>
                  <?php endif; ?>
                  <?php if(in_array('staff5',staff_role_resource()) || ($user_info['user_type'] ?? '') == 'company'): ?>
                  <a href="#!" class="dropdown-item delete" data-toggle="modal" data-target=".delete-modal" data-record-id="<?= uencode($r['user_id']) ?>">
        ...
        <div class="text-center">
          <a href="<?= site_url('erp/employee-details/'.uencode($r['user_id'])) ?>">
            <h4 class="mb-1 mt-3">
          ...
          <p class="mb-1">
            <b><?= lang('Main.xin_email') ?>: </b>
            <a href="mailto:<?= htmlspecialchars($r['email']) ?>">
              <?= htmlspecialchars($r['email']) ?>
            </a>
          </p>
          <p class="mb-0">
            <b><?= lang('Main.xin_employee_role') ?>: </b>
            <?= htmlspecialchars($role['role_name']) ?>
            <span class="badge badge-primary">
              <?= htmlspecialchars($office_shifts['shift_name']) ?>
            </span>
          </p>
        </div>
      </div>
