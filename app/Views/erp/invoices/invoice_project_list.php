...
else: $unpaid = $unpaid / $count_invoices * 100;
endif;
$unpaid = number_format((float)$unpaid, 1, '.', '');

if ($count_invoices < 1): $paid = 0;
else: $paid = $paid / $count_invoices * 100;
endif;
    ...
    <div class="row">
      <?php foreach ($get_invoices as $r) { ?>
        ...
        if (!empty($client_info)) {
          $displayName = $client_info['first_name'] . ' ' . $client_info['last_name'];
        } elseif (!empty($expertData)) {
