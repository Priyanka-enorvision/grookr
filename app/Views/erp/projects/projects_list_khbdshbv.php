...
if ($user_info['user_type'] == 'staff') {
  ...
  $project_list = $ProjectsModel->where('company_id', $company_id)->groupStart()->where('added_by', $user_id)->orWhere('FIND_IN_SET(' . $user_id . ', assigned_to) > 0')->groupEnd()->findAll();
    ...
    <tbody>
      ...
      foreach ($project_list as $list) { ?>
        <tr class="has-row-options odd" role="row">
          ...
          <td><?= number_to_currency($list['revenue'] ?? 0, $xin_system['default_currency'], 'en_US', 2); ?></td> -->
