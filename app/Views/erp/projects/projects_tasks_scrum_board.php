...
if($user_info['user_type'] == 'staff'){
  $staff_info = $UsersModel->where('company_id', $user_info['company_id'])->where('user_type','staff')->findAll();
	...
	$get_data = $staff_tasks_board['data'];
	$not_started_tasks = $staff_tasks_board['not_started_tasks'];
  $not_started = empty($not_started_tasks) ? 0 : count($not_started_tasks);
	$inprogress_tasks = $staff_tasks_board['inprogress_tasks'];
  $in_progress = empty($inprogress_tasks) ? 0 : count($inprogress_tasks);
	$completed_tasks = $staff_tasks_board['completed_tasks'];
  $completed = empty($completed_tasks) ? 0 : count($completed_tasks);
	$cancelled_tasks = $staff_tasks_board['cancelled_tasks'];
  $cancelled = empty($cancelled_tasks) ? 0 : count($cancelled_tasks);
	$hold_tasks = $staff_tasks_board['hold_tasks'];
  $hold = empty($hold_tasks) ? 0 : count($hold_tasks);
  ...
  <div class="dropdown">
    ...
    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="margin-right: 5rem !important;">
      <?php if(in_array('tasks_calendar',staff_role_resource()) || $user_info['user_type'] == 'company') {?>
        <a href="<?= site_url('erp/tasks-calendar');?>" class="dropdown-item"> 
          <span class="sw-done-icon feather icon-check-circle"></span> 
          <span class="sw-icon feather icon-calendar"></span>
      ...
      <?php if(in_array('tasks_sboard',staff_role_resource()) || $user_info['user_type'] == 'company') {?>
        <a href="<?= site_url('erp/tasks-scrum-board');?>" class="dropdown-item"> 
          <span class="sw-done-icon feather icon-check-circle"></span> 
          <span class="sw-icon fas fa-tasks"></span>
      ...
      <div class="card mb-3">
          ...
          <div class="kanban-box first-notstarted px-2 pt-2" id="first-notstarted" data-status="0">
              <?php foreach ($not_started_tasks as $hltask) { ?>
                  <?php if ($hltask['task_status'] == 0) { ?>
                      ...
                      $progress_class = get_progress_class($hltask['task_progress']);
                      ?>
                      <div class="ui-bordered notstarted_<?php echo htmlspecialchars($hltask['task_id']); ?> p-2 mb-2"
                          data-id="<?php echo htmlspecialchars($hltask['task_id']); ?>"
                          data-status="0"
                          data-project-id="<?php echo htmlspecialchars($hltask['project_id']); ?>"
                          data-assigned-to="<?php echo htmlspecialchars(implode(',', $cc)); ?>"
                          data-experted-to="<?php echo htmlspecialchars($hltask['expert_to']);?>"
                          id="notstarted">
                          <a href="<?= site_url('erp/task-detail') . '/' . uencode($hltask['task_id']); ?>">
                              <?php echo htmlspecialchars($hltask['task_name']); ?>
                          </a>
                          <div>
                              <small class="text-muted"><?= lang('Projects.xin_completed'); ?> <?php echo htmlspecialchars($hltask['task_progress']); ?>%</small>
                          </div>
                          <div class="progress" style="height: 10px;">
                              <div class="progress-bar progress-bar-striped <?php echo htmlspecialchars($progress_class); ?>"
                                  style="width: <?php echo htmlspecialchars($hltask['task_progress']); ?>%"></div>
                          </div>
                          <div class="text-muted small mb-1 mt-2"><?= lang('Projects.xin_project_users'); ?></div>
                          <div class="d-flex flex-wrap mt-1"><?= multi_user_profile_photo($cc); ?></div>
                      </div>
                  <?php } ?>
              <?php } ?>
          </div>
          <?php if (in_array('task2', staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
              <div class="card-footer text-center py-2">
                  <a href="<?= site_url('erp/tasks-list'); ?>" class="edit-data add-task">
        ...
        <div class="kanban-box first-inprogress px-2 pt-2" id="first-inprogress" data-status="1">
            <?php foreach ($inprogress_tasks as $hltask) { ?>
                <?php if ($hltask['task_status'] == 1) { ?>
                    ...
                    $progress_class = get_progress_class($hltask['task_progress']);
                    ?>
                    <div class="ui-bordered in-progress_<?php echo htmlspecialchars($hltask['task_id']); ?> p-2 mb-2"
                        data-id="<?php echo htmlspecialchars($hltask['task_id']); ?>"
                        data-status="1"
                        data-project-id="<?php echo htmlspecialchars($hltask['project_id']); ?>"
                        data-assigned-to="<?php echo htmlspecialchars(implode(',', $cc)); ?>"
        ...
        <div class="kanban-box first-completed px-2 pt-2" id="first-completed" data-status="2">
            <?php foreach ($completed_tasks as $hltask) { ?>
                <?php if ($hltask['task_status'] == 2) { ?>
                    ...
                    $progress_class = get_progress_class($hltask['task_progress']);
                    ?>
                    <div class="ui-bordered complete_<?php echo htmlspecialchars($hltask['task_id']); ?> p-2 mb-2"
                        data-id="<?php echo htmlspecialchars($hltask['task_id']); ?>"
                        data-project-id="<?php echo htmlspecialchars($hltask['project_id']); ?>"
                        data-assigned-to="<?php echo htmlspecialchars(implode(',', $cc)); ?>"
                        data-experted-to="<?php echo htmlspecialchars($hltask['expert_to']);?>"
                        ...
                        <div>
                            <small class="text-muted">
                                <?= lang('Projects.xin_completed'); ?>
                                <?php echo htmlspecialchars($hltask['task_progress']); ?>%
                            </small>
                        </div>
                        <div class="progress" style="height: 10px;">
                            <div class="progress-bar progress-bar-striped <?php echo htmlspecialchars($progress_class); ?>"
        ...
        <div class="kanban-box first-cancelled px-2 pt-2" id="first-cancelled" data-status="3">
            <?php foreach ($cancelled_tasks as $hltask) { ?>
                <?php if ($hltask['task_status'] == 3) { ?>
                    ...
                    $progress_class = get_progress_class($hltask['task_progress']);
                    ?>
                    <div class="ui-bordered cancelled_<?php echo htmlspecialchars($hltask['task_id']); ?> p-2 mb-2"
                        data-id="<?php echo htmlspecialchars($hltask['task_id']); ?>"
                        data-status="3"
                        data-project-id="<?php echo htmlspecialchars($hltask['project_id']); ?>"
                        data-assigned-to="<?php echo htmlspecialchars(implode(',', $cc)); ?>"
        ...
        <div class="kanban-box first-hold px-2 pt-2" id="first-hold" data-status="4">
            <?php foreach ($hold_tasks as $hltask) { ?>
                <?php if ($hltask['task_status'] == 4) { ?>
                    ...
                    $progress_class = get_progress_class($hltask['task_progress']);
                    ?>
                    <div class="ui-bordered hold_<?php echo htmlspecialchars($hltask['task_id']); ?> p-2 mb-2"
                        data-id="<?php echo htmlspecialchars($hltask['task_id']); ?>"
                        data-status="4"
                        data-project-id="<?php echo htmlspecialchars($hltask['project_id']); ?>"
                        data-assigned-to="<?php echo htmlspecialchars(implode(',', $cc)); ?>"
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    function filterTasks() {
        ...
        allTasks.forEach(task => {
            const taskProject = task.getAttribute("data-project-id");
            ...
            if (projectId && taskProject !== projectId) {
                isVisible = false;
            }
