...
<div class="dashboard-container">
    ...
    <div class="monthly-summary" id="planningEntitiesContainers">
        <!-- Dynamic entity list will be added here -->
    </div>
</div>
<!--  -->
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function loadChartData() {
            ...
            $.ajax({
                url: main_url + 'invoices/month_plan_chart',
                type: 'GET',
                ...
                success: function (response) {
                    ...
                    if (response && response.invoice_month && response.paid_invoice && response.unpaid_invoice) {
                        ...
                        if (Array.isArray(response.monthly_planning_entities)) {
                            // Dynamic generation of planning entities
                            const planningEntitiesContainers = document.getElementById('planningEntitiesContainers');
                            planningEntitiesContainers.innerHTML = '';  // Clear any previous content
                            response.monthly_planning_entities.forEach(entity => {
                                const entityDiv = document.createElement('div');
                                entityDiv.classList.add('status-item');
                                ...
                                entityDiv.appendChild(entityName);
                                entityDiv.appendChild(entityValue);
                                planningEntitiesContainers.appendChild(entityDiv);
                            });
                        } else {
                            console.error("monthly_planning_entities is not an array or is missing from the response.");
                        }
