      ...
      <div>
        <div class="card" id="printTable">
          <div class="card-header">
            <h5>
              <?= lang('Main.xin_view_estimate'); ?>
            </h5>
            <div class="card-header-right"> <a href="<?= site_url('erp/print-estimate/') . $invoice_id; ?>" target="_blank" class="collapsed btn btn-sm waves-effect waves-light btn-success m-0">
                <?= lang('Invoices.xin_print_download_invoice'); ?>
              </a>
              <?php if ($user_info['user_type'] != 'customer') { ?>
                <?php if ($result['status'] == 0) { ?>
                  <?php if (in_array('estimate6', staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                    <button type="button" class="btn btn-sm btn-info" data-toggle="modal" data-field_id="<?php echo $invoice_id; ?>" data-target=".payroll-modal-data">
                      <?= lang('Main.xin_convert_estimate_to_invoice'); ?>
                    </button>
                  <?php } ?>
                  <?php if (in_array('estimate7', staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                    <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-field_id="<?php echo $invoice_id; ?>" data-target=".view-modal-data">
                      <?= lang('Main.xin_cancel_estimate'); ?>
                    </button>
                  <?php } ?>
                  <?php if (in_array('estimate4', staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
                    <a href="<?= site_url('erp/edit-estimate/') . $invoice_id; ?>" class="collapsed btn btn-sm waves-effect waves-light btn-primary m-0">
                      <?= lang('Main.xin_edit'); ?>
                    </a>
                  <?php } ?>
                <?php } ?>
