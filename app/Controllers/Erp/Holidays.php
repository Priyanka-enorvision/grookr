...
class Holidays extends BaseController
{
	...
	public function index()
	{
		...
		$data['breadcrumbs'] = lang('Dashboard.left_holidays');
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			$rules = [
				'event_name' => [
					...
					'errors' => [
						'required' => lang('Main.xin_error_field_text'),
						'regex_match' => "Invalid Event Name"
					]
				],
				'start_date' => [
					'rules' => 'required',
			...
			if (!$this->validate($rules)) {
				...
				$HolidaysModel = new HolidaysModel();
				$existingRecord = $HolidaysModel->where('event_name', $event_name)
					->where('start_date', $start_date)
					->where('end_date', $end_date)
					->first();

				if ($existingRecord) {
					$Return['error'] = "This Holiday already exists"; // Custom error for duplicates
					$this->output($Return);
					exit;
				}
				...
				$data = [
					...
					'description'  => $description,
					'is_publish'  => $is_publish,
					'created_at' => date('d-m-Y h:i:s') // Standardized date format
				];
