...
class Profile extends BaseController
{
	...
	public function update_profile_photo()
	{
		...
		// Basic validations
		if (!$this->request->isAJAX() || $this->request->getPost('type') !== 'edit_record' || !$usession || !isset($usession['sup_user_id'])) {
		...
		if ($file->isValid() && !$file->hasMoved()) {
			...
			$thumbPath = FCPATH . 'uploads/users/thumb/';
			...
			// Create thumbnail with same name
			\Config\Services::image()
				->withFile($uploadPath . $originalName)
				->fit(100, 100, 'center')
				->save($thumbPath . $originalName);

			// Save original filename in database
			$data = ['profile_photo' => $originalName];

			if ($UsersModel->update($userId, $data)) {
				$response['result'] = lang('Main.xin_profile_picture_success_updated');
				$response['file_name'] = $originalName;
				$session->set('sup_user_profile_photo', $originalName);
			} else {
				$response['error'] = lang('Main.xin_error_msg');
				// Clean up if DB fails
				@unlink($uploadPath . $originalName);
				@unlink($thumbPath . $originalName);
			}
		} else {
			$response['error'] = $file->getErrorString() ?: lang('Main.xin_error_msg');
		}
	...
	// 	if ($this->request->getPost('type') === 'edit_record') {
	// 		$response = ['success' => false, 'message' => '', 'errors' => [], 'csrf_hash' => csrf_hash()];
	...
	// 			'system_timezone' => 'required',
	// 			'default_language' => 'required',
	// 			'invoice_terms_condition' => 'required|min_length[10]',
	// 		];
	...
	// 			'invoice_terms_condition' => [
	// 				'required' => lang('Main.xin_invoice_terms_condition_error_field'),
	// 				'min_length' => lang('Main.xin_invoice_terms_min_length'),
	// 			]
	// 		];

	// 		if (!$this->validate($rules, $messages)) {
	// 			$response['errors'] = $validation->getErrors();
	// 			$response['message'] = lang('Main.xin_error_msg');
	// 			return $this->response->setJSON($response);
	// 		}
	...
	// 			'date_format_xi' => $this->request->getPost('date_format'),
	...
	// 			$response['message'] = lang('Success.xin_account_settings_updated_success');
	// 		} else {
	// 			$response['message'] = lang('Main.xin_error_msg');
	// 		}
	...
	public function notification_position_info()
	{
		...
		if ($notification_close == ''): $notification_close = 0;
		endif;
		if ($notification_bar == ''): $notification_bar = 0;
		endif;
		$id = udecode($this->request->getPost('token', FILTER_SANITIZE_STRING));
		$data = [
