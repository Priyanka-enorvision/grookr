...
class Types extends BaseController
{
	...
	public function apply_performance()
	{
		...
		$data['title'] = 'Apply Performance' . ' | ' . $xin_system['application_name'];
		$data['path_url'] = 'staff_performance';
		$data['breadcrumbs'] = lang('Dashboard.xin_category');
	...
	public function add_leave_type()
	{
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			$rules = [
				...
				'days_per_year' => [
					'rules' => 'required|integer|greater_than_equal_to[1]|less_than_equal_to[30]',
					'errors' => [
						'required' => lang('Main.xin_error_field_text'),
						'integer' => 'Days per year must be an integer.',
						'greater_than_equal_to' => 'Minimum day value is 1.',
						'less_than_equal_to' => 'Maximum days value is 30.'
					]
				]
			];
			...
			if (!$this->validate($rules)) {
				...
				$ConstantsModel = new ConstantsModel();
				$existing_leave_type = $ConstantsModel->where([
					'company_id' => $company_id,
					'category_name' => $name,
					'type' => 'leave_type'
				])->first();
	...
	public function add_competencies()
	{
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if (!$this->validate($rules)) {
				...
				if ($result == TRUE) {
					$Return['result'] = lang('Success.ci_category_added_msg');
					$Return['redirect_url'] = base_url('erp/competencies');
				} else {
					$Return['error'] = lang('Main.xin_error_msg');
				}
				return $this->response->setJSON($Return);
	...
	public function update_constants_type()
	{
		...
		if ($this->request->getPost()) {
			...
			if (!$this->validate($rules)) {
				...
				$ConstantsModel = new ConstantsModel();
				$duplicateCheck = $ConstantsModel->where('category_name', $name)
		...
		$rules = [
			...
			'days_per_year' => [
				...
				'errors' => [
					...
					'greater_than_equal_to' => lang('Main.xin_error_field_positive')
				]
			]
		];

		if (!$this->validate($rules)) {
			$Return['error'] = $validation->getError() ?: lang('Main.xin_error_msg');
			return $this->response->setJSON($Return);
		}

		try {
			$name = $this->request->getPost('name', FILTER_SANITIZE_STRING);
			$days_per_year = $this->request->getPost('days_per_year', FILTER_SANITIZE_NUMBER_INT);
			$requires_approval = $this->request->getPost('requires_approval', FILTER_SANITIZE_STRING);
			...
			$data = [
				...
				'field_one' => $days_per_year,
				'field_two' => $requires_approval,
				'company_id' => $company_id // added company_id if needed for updates
			];
			...
			$Return['error'] = lang('Main.xin_error_msg');
			log_message('error', 'Error updating leave type: ' . $e->getMessage());
		}
