...
class Estimates extends BaseController
{
	...
	//estimate_details
	public function estimate_details($ifield_id)
	{
		$session = \Config\Services::session();
		$SystemModel = new SystemModel();
		$UsersModel = new UsersModel();
	...
	public function update_estimate()
	{
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if ($this->request->getPost('item')) {
				$item_names = $this->request->getPost('eitem_name', FILTER_SANITIZE_STRING) ?? [];
				$qtys = $this->request->getPost('eqty_hrs', FILTER_SANITIZE_STRING) ?? [];
				$unit_prices = $this->request->getPost('eunit_price', FILTER_SANITIZE_STRING) ?? [];
				$sub_total_items = $this->request->getPost('esub_total_item', FILTER_SANITIZE_STRING) ?? [];

				foreach ($this->request->getPost('item', FILTER_SANITIZE_STRING) as $eitem_id => $key_val) {
					$iname = $item_names[$key_val] ?? '';
					$qtyhrs = $qtys[$key_val] ?? '';
					$price = $unit_prices[$key_val] ?? '';
					$item_sub_total = $sub_total_items[$key_val] ?? '';
					...
					if ($iname === '') {
						...
						$Return['error'] = lang('Success.xin_price_field_error');
					}
			...
			if ($this->request->getPost('item_name')) {
				$item_names = $this->request->getPost('item_name', FILTER_SANITIZE_STRING) ?? [];
				$qtys = $this->request->getPost('qty_hrs', FILTER_SANITIZE_STRING) ?? [];
				$unit_prices = $this->request->getPost('unit_price', FILTER_SANITIZE_STRING) ?? [];

				foreach ($item_names as $k => $iname) {
					$qtyhrs = $qtys[$k] ?? '';
					$price = $unit_prices[$k] ?? '';
			...
			$_project = $ProjectsModel->where('company_id', $company_id)
			...
			$_client = $UsersModel->where('user_id', $_project['client_id'])
			...
			if ($result) {
				...
				if ($this->request->getPost('item_name')) {
					...
					$qtys = $this->request->getPost('qty_hrs', FILTER_SANITIZE_STRING) ?? [];
					$unit_prices = $this->request->getPost('unit_price', FILTER_SANITIZE_STRING) ?? [];
					$sub_total_items = $this->request->getPost('sub_total_item', FILTER_SANITIZE_STRING) ?? [];
					...
					foreach ($item_names as $k => $iname) {
						$data3 = [
							...
							'item_name' => $iname,
							'item_qty' => $qtys[$k] ?? '',
							'item_unit_price' => $unit_prices[$k] ?? '',
							'item_sub_total' => $sub_total_items[$k] ?? '',
							'created_at' => date('d-m-Y H:i:s')
						];
