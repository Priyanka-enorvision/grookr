...
class Policies extends BaseController
{
	...
	public function staff_policies_all()
	{
		...
		$data['path_url'] = 'policies';
		$data['breadcrumbs'] = lang('Dashboard.xin_view_policies');
	...
	public function policies_list()
	{
		...
		foreach ($get_data as $r) {
			...
			$pre_url = base_url() . '/public/uploads/policy/' . $r['attachment'];
			$url = '<div class="d-inline-block align-middle">
				<div class="d-inline-block">
				<a href="' . $pre_url . '">
					<h6 class="m-b-0">' . $r['attachment'] . '</h6>
				<a>
				</div>
			</div>';
			$data[] = array(
	...
	public function add_policy()
	{
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			$rules = [
				...
				'attachment' => [
					'rules'  => 'uploaded[attachment]|mime_in[attachment,application/pdf, image/jpg,image/jpeg,image/gif,image/png]|max_size[attachment,30072]',
					'errors' => [
		...
		if ($this->request->getPost('type') === 'edit_record') {
			...
			if (!$this->validate($rules)) {
				...
				$validated = $this->validate([
					'attachment' => [
						'rules'  => 'uploaded[attachment]|mime_in[attachment,application/pdf, image/jpg,image/jpeg,image/gif,image/png]|max_size[attachment,3072]',
						'errors' => [
							'uploaded' => lang('Main.xin_error_field_text'),
							'mime_in' => 'wrong size'
						]
