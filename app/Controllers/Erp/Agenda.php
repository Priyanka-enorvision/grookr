...
class Agenda extends BaseController {
	...
	public function leave_list($id=null) {
		...
		$data = [];
          ...
          foreach($get_data as $r) {
			...
			$uname = '<div class="d-inline-block align-middle">
				<img src="'.base_url().'uploads/users/thumb/'.$staff['profile_photo'].'" alt="'.$name.'" class="img-radius align-top m-r-15" style="width:40px;">
	 ...
	 // record list
	public function expense_list($id=null) {
		...
		$data = [];
          ...
          foreach($get_data as $r) {
			...
			$iaccount_name = $iaccounts['account_name'];
			// $iaccount_name = '
	 ...
	 // record list
	public function loan_list($id=null) {
	 ...
	 // record list
	public function travel_list($id=null) {
		...
		$data = [];
          ...
          foreach($get_data as $r) {						
			...
			$t_employee_name = $iemployee_name;
	...
	public function projects_list() {
	 ...
	 // record list
	public function tasks_list($id=null) {
	...
	public function payslip_history_list() {
		...
		$data = [];
          ...
          foreach($payslip as $r) {						
				...
				$uname = '<div class="d-inline-block align-middle">
					<img src="'.base_url().'uploads/users/thumb/'.$user_detail['profile_photo'].'" alt="user image" class="img-radius align-top m-r-15" style="width:40px;">
					<div class="d-inline-block">
						<h6 class="m-b-0">'.$name.'</h6>
