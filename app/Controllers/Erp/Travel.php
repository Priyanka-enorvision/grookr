...
class Travel extends BaseController
{
	...
	public function index()
	{
		...
		$data['breadcrumbs'] = lang('Dashboard.left_travels');
	...
	public function travel_details()
	{
		...
		$data['breadcrumbs'] = lang('Main.xin_travel_details');
	...
	public function update_travel()
	{
		...
		try {
			...
			if (strtotime($end_date) < strtotime($start_date)) {
				$Return['errors']['end_date'] = lang('Main.xin_error_end_date');
				return $this->response->setJSON($Return);
			}

			$visit_purpose = esc($this->request->getPost('visit_purpose'));
			$visit_place = esc($this->request->getPost('visit_place'));
			$expected_budget = (float)$this->request->getPost('expected_budget');
			$actual_budget = (float)$this->request->getPost('actual_budget');
			$arrangement_type = esc($this->request->getPost('arrangement_type'));
			$travel_mode = esc($this->request->getPost('travel_mode'));
			$description = esc($this->request->getPost('description'));

			// Handle associated goals
			$associated_goals = $this->request->getPost('associated_goals');
			$associated_goals_str = '';

			if (!empty($associated_goals) && is_array($associated_goals)) {
				$associated_goals = array_map('esc', $associated_goals);
				$associated_goals_str = implode(',', $associated_goals);
			}
			...
			$data = [
				...
				'actual_budget' => $actual_budget,
				'description' => $description,
				'associated_goals' => $associated_goals_str,
				'updated_at' => date('Y-m-d H:i:s')
			];

			$TravelModel = new TravelModel();
			$result = $TravelModel->where('travel_id', $id)->update($id, $data);

			if ($result) {
				$Return['result'] = "Travel Updated Successfully";
			} else {
				$Return['error'] = lang('Main.xin_error_msg');
			}
		} catch (\Exception $e) {
			log_message('error', 'Travel update error: ' . $e->getMessage());
			$Return['error'] = lang('Main.xin_error_msg');
		}
	...
	public function travel_list()
	{
		...
		foreach ($get_data as $r) {
			...
			elseif ($r['status'] == 1): $status = '<span class="badge badge-success">' . lang('Main.xin_accepted') . '</span>';
			else: $status = '<span class="badge badge-danger">' . lang('Main.xin_rejected');
			endif;
