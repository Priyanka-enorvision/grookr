...
class Languages extends BaseController
{
	...
	public function languages_list()
	{
		...
		foreach ($languages as $r) {
			...
			$flag = '<img width="16" height="11" src="' . base_url() . 'uploads/languages_flag/' . $r['language_flag'] . '">';
	...
	public function add_language()
	{
		...
		if ($this->request->getPost('type') === 'add_record') {
			...
			if (!$validated) {
				...
				// Move to temp first
				$avatar->move(WRITEPATH . 'uploads/languages_flag/temp/', $file_name);

				// Process image
				$image->withFile(WRITEPATH . 'uploads/languages_flag/temp/' . $file_name)
					->fit(16, 11, 'center')
					->save(WRITEPATH . 'uploads/languages_flag/' . $file_name);

				// Clean up temp file
				unlink(WRITEPATH . 'uploads/languages_flag/temp/' . $file_name);
			}
			...
			$new_dir = APPPATH . 'Language/' . $language_code;
			...
			if (!is_dir($new_dir)) {
				...
				file_put_contents($new_dir . '/index.html', '');
			}

			// List of language files to copy from English
			$language_files = [
				'Company.php',
				'Language.php',
				'Asset.php',
			...
			// Copy each language file from English to new language directory
			foreach ($language_files as $file) {
				$source = APPPATH . 'Language/en/' . $file;
	...
	public function language_status($field_id, $is_active)
	{
		$request = \Config\Services::request();
